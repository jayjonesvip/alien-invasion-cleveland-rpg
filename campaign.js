...
function chooseDispatchStreet(street) {
  ...
  Game.dispatchSelection=street;
  showDispatchStreet(street);
}
function goToDispatchStreet(street) {
  if(Game.scene!=='dispatch'||Game.dispatchSelection!==street||StoryType.holdLock||StoryType.typing)return;
...
function showDispatchStreet(street) {
...
function showDirectory(arrival='') {
  ...
  if(cleared){
    const dispatchButton=addButton('Get in Cab',()=>showDispatch());
...
function showDepartures() {
  ...
  describeActionButton(walk,'Trust your own two feet.');
  const bus=addButton('Take the bus',boardBus);
  describeActionButton(bus,'Ride while service lasts.');
  const rapid=addButton('Take the Rapid',boardRapid);
  describeActionButton(rapid,'Take your chances below.');
  const cab=addButton('Hail a cab',boardCab);
  describeActionButton(cab,'Trust the driver.');
}
function boardCab() {
  if(Math.random()<0.5){
