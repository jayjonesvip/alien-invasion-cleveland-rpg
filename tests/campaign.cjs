...
Game.scene='combat';Game.enemy=makeEnemy(false);Game.abilitiesUsed={};Game.money=0;updateCombatButtons();el('#buttons').children.find(b=>b.textContent==='Flee').onclick({});assert.equal(Game.money,0);const aid=el('#buttons').children.find(b=>b.textContent.startsWith('Take $'));assert.ok(aid);aid.onclick({});assert.ok(Game.money>0,'Emergency cash waits for Take');assert.ok(!el('#buttons').children.some(b=>b.textContent==='Keep Exploring'),'Fleeing cannot immediately redeal the fight');
Game.level=Game.highestDistrict=4;Game.currentStreet=4;Game.route=defaultRoute();Game.scene='combat';Game.enemy=makeEnemy(false);Game.hp=1;Game.money=5;Game.knownShops=[];Game.emergencyDogsUsed=[];updateCombatButtons();el('#buttons').children.find(b=>b.textContent==='Flee').onclick({});const dog=el('#buttons').children.find(b=>b.textContent==='Eat Emergency Hot Dog');assert.ok(dog,'Low-health player without healing gets emergency food');dog.onclick({});assert.equal(Game.hp,Math.ceil(Game.maxHP*.4));assert.ok(Game.emergencyDogsUsed.includes(4));
